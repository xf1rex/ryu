...
class Ecmp13(app_manager.RyuApp):
    ...
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst):
        ...
        #paths = nx.all_shortest_paths(self.net, source=src, target=dst)
        switches_in_paths = self.find_switches(paths_with_ports, src, dst)
    ...
    def _packet_in_handler(self, ev):
        ...
        if not dst_ip in self.arp_table:
            ...
            if dst == None:
                ...
                """
                print "ETH SRC "+src
                print ("SRC PORT ",self.mac_to_port[dpid][src])
                print "ETH DST "+dst
                print ("DST PORT ",self.mac_to_port[dst_dpid][dst])
                print "IP SRC "+src_ip
                print "IP DST "+dst_ip
                """
                out_port = self.install_paths(src, self.mac_to_port[dpid][src], dst, self.mac_to_port[dst_dpid][dst], src_ip, dst_ip)
